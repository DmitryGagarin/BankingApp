...
const Header = ({ name, surname, leaveAccount, goToHome, createDeposit, allDeposits, topUpDeposit }) => {
    return (
        <header style={{ backgroundColor: 'green', display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 20px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center' }}>
                ...
                <button onClick={topUpDeposit} style={{ margin: '0 10px' }}>Top up Deposit</button>
            </div>
        </header>
    );
};
